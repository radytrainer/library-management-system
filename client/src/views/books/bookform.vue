...
const emit = defineEmits([
  ...
  'authorUpdated',
  'categoryUpdated',
  'updateForm' // New event for updating form state
])
...
const submitAuthorForm = async () => {
  ...
  try {
    ...
    console.log('Author FormData:', [...formData.entries()].map(([k, v]) => ({ key: k, value: v instanceof File ? `File: ${v.name}` : v })))
    ...
    updatedAuthor.profile_image_url = updatedAuthor.profile_image_url
      ? `${updatedAuthor.profile_image_url}?t=${Date.now()}`
      : null
    emit('authorUpdated', updatedAuthor)
    emit('updateForm', { AuthorId: updatedAuthor.id }) // Emit to update form
    closeAuthorForm()
    Swal.fire({
      toast: true,
      position: 'bottom-end',
...
const submitCategoryForm = async () => {
  ...
  try {
    ...
    emit('updateForm', { CategoryId: updatedCategory.id }) // Emit to update form
    closeCategoryForm()
    Swal.fire({
      toast: true,
      position: 'bottom-end',
...
watch(() => props.form.AuthorId, (newVal) => {
  if (newVal === 'add-new-author') {
    openAuthorForm(null)
    emit('updateForm', { AuthorId: '' }) // Update form in parent
  }
})
...
watch(() => props.form.CategoryId, (newVal) => {
  if (newVal === 'add-new-category') {
    ...
    emit('updateForm', { CategoryId: '' }) // Update form in parent
  }
})
