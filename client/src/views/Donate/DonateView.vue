<template>
  <div class="max-w-7xl mx-auto space-y-6 p-8">
    <!-- Header with Static Book Image -->
    <div
      class="w-full bg-sky-500 rounded-xl shadow-md p-12 flex flex-col md:flex-row md:justify-between justify-center gap-12 py-5"
    >
      <div class="flex flex-col gap-4 pr-4 md:pr-12">
        <div>
          ...
          <p class="text-sm text-white mt-3">
            ...
            Use powerful search and filter tools to quickly find titles, check
            availability, and keep the inventory up to date. Whether you're
            donating, editing, or reviewing book details, everything you need is
      ...
      <div class="flex justify-end items-center p-6">
        <div class="relative w-40 h-40 overflow-hidden rounded-xl shadow-md">
          <img
            src="https://i.pinimg.com/736x/d1/0e/91/d10e9103d8cb653c62a0d87adaf171cd.jpg"
    ...
    <div class="flex flex-col sm:flex-row justify-between items-center gap-4">
      <div class="w-full sm:w-1/2">
        <input
          v-model="searchTerm"
          type="text"
          placeholder="Search by title, ISBN, or owner..."
          class="w-full border border-gray-400 rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-indigo-500 focus:outline-none"
        />
      </div>
      <button
        @click="openAddDialog"
        class="flex items-center gap-2 px-5 py-2.5 bg-sky-500 text-white rounded-md hover:bg-blue-400 text-sm shadow"
    ...
    >
      <div
        ...
        :key="book.id"
        class="bg-white shadow border border-gray-200 rounded-xl overflow-hidden hover:shadow-md transition-shadow duration-300 cursor-pointer"
        @click="viewBook(book)"
      >
        <div class="relative">
          <img
          ...
          >
            ...
            <button
              @click="
                ...
                closeMenu();
              "
              class="block w-full text-left px-3 py-1 text-xs text-red-600 hover:bg-red-50"
            >
              Delete
            </button>
          </div>
        ...
        <div class="p-4 space-y-2 text-sm">
          ...
          <span
            class="inline-block px-2 py-0.5 bg-indigo-100 text-indigo-700 rounded-full text-[10px] font-medium"
          >
            Qty: {{ book.quantity }}
          </span>
    ...
    <div v-if="canShowMore" class="flex justify-center mt-6">
      <button
        @click="showMore"
        class="px-6 py-2 bg-indigo-500 text-white rounded-md hover:bg-indigo-600 transition shadow"
      >
        Show More
      </button>
    </div>
    ...
    <div
      ...
      class="text-center text-gray-500 italic p-6 w-full"
    >
      No books found.
    </div>
    ...
    >
      <div
        role="dialog"
        aria-modal="true"
        class="bg-white p-5 sm:p-6 rounded-xl w-full max-w-md shadow-lg relative"
      >
        <button
          class="absolute top-2.5 right-3 text-gray-400 hover:text-gray-800 text-xl"
          @click="closeDialog"
        ...
        <div v-if="isViewing" class="space-y-3 text-sm">
          ...
          <div>
            ...
            {{ getLanguageName(donation.language_id) }}
          </div>
          <div>
            <span class="font-medium text-gray-700">Category:</span>
            {{ getCategoryName(donation.category_id) }}
          </div>
          <div>
            ...
            <img
              :src="
                donation.book_image ||
                'https://i.pinimg.com/736x/d1/0e/91/d10e9103d8cb653c62a0d87adaf171cd.jpg'
              "
              alt="Book Cover"
              class="h-28 object-contain rounded border mt-1"
            />
          </div>
          <div class="flex justify-end pt-2">
            <button
              @click="closeDialog"
              class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-1.5 rounded-md font-medium shadow-sm transition"
            >
              Close
            </button>
          </div>
        ...
        <form v-else @submit.prevent="submitDonation" class="space-y-3 text-sm">
          <div>
            ...
            ><input
              ...
              required
              class="w-full border px-3 py-1.5 rounded-md"
            />
          </div>
          <div>
            <label class="block mb-1 text-gray-700">ISBN</label
            ><input
              v-model="donation.isbn"
              required
              class="w-full border px-3 py-1.5 rounded-md"
            />
          </div>
          <div>
            ...
            ><input
              v-model.number="donation.quantity"
              type="number"
              min="1"
              required
              class="w-full border px-3 py-1.5 rounded-md"
          ...
          <div>
            <label class="block mb-1 text-gray-700"
              >Cover Image URL (optional)</label
            >
            <input
              v-model="donation.book_image"
              type="text"
              placeholder="Enter image URL"
              class="w-full border px-3 py-1.5 rounded-md"
            />
            <div v-if="donation.book_image" class="mt-2">
              <img
                :src="donation.book_image"
                alt="Preview"
                class="h-28 object-contain rounded border"
              />
            </div>
          </div>
          <div class="flex justify-end pt-2">
...
const visibleBooks = computed(() =>
  sortedBooks.value.slice(0, visibleCount.value)
);
const canShowMore = computed(
  () => visibleCount.value < sortedBooks.value.length
);
const showMore = () => (visibleCount.value += 8);

const getLanguageName = (id) =>
  ({ 1: "English", 2: "French", 3: "Khmer" }[id] || "Unknown");
const getCategoryName = (id) =>
  ({ 1: "Fiction", 2: "Classic", 3: "Science" }[id] || "Unknown");
</script>
