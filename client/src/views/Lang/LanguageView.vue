...
const fetchInitialData = async (retries = 3) => {
  try {
    ...
    if (retries > 0) {
      ...
      error.value = 'Failed to load data after multiple attempts. Please check your network or server status.';
...
const filteredLanguages = computed(() => {
  ...
  return languages.value.filter(lang => !searchTerm || lang.name.toLowerCase().includes(searchTerm));
});
...
const filterBook = (book) => {
  ...
  return book?.title?.toLowerCase().includes(searchTerm) || book?.language?.toLowerCase().includes(searchTerm);
};
...
const confirmDelete = async () => {
  try {
    const deletedLang = allLanguages.value.find(l => l?.id === languageToDelete.value)?.name;
    const languageBooksCount = books.value.filter(b => b?.language === deletedLang).length;
    ...
    allLanguages.value = allLanguages.value.filter(l => l?.id !== languageToDelete.value);
