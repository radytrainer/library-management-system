<template>
  <div class="bg-gradient-to-br from-white to-gray-50 rounded-3xl p-8 shadow-lg border border-gray-200/50 mb-8 backdrop-blur-sm">
    <!-- Header Section -->
    <div class="flex items-center justify-between mb-8">
      <div class="flex items-center gap-3">
        <div class="w-2 h-8 bg-gradient-to-b from-amber-400 to-orange-500 rounded-full"></div>
        <h3 class="text-2xl font-bold text-gray-900 tracking-tight">Limited Availability</h3>
      </div>
      <button 
        @click="goToLimitedBooks" 
        class="group flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-xl transition-all duration-200 hover:shadow-lg hover:scale-105"
      >
        <span>View All</span>
    ...
    <!-- Books Grid -->
    <div v-else-if="limitedBooks.length > 0" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
      <div 
      ...
      >
        ...
        <div class="flex-1 w-full">
          <h4 class="font-bold text-gray-900 text-sm mb-1 line-clamp-2 group-hover:text-blue-700 transition-colors duration-200">
            {{ book.title }}
          </h4>
        ...
        <div class="absolute inset-0 bg-gradient-to-t from-blue-600/5 to-transparent rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none"></div>
      </div>
    </div>
