      ...
      <form @submit.prevent="submitAuthorForm" class="p-6">
        ...
        <div class="flex justify-end gap-3 mt-6 pt-4 border-t border-gray-100">
          ...
          <button type="submit" class="btn-primary" :disabled="isSubmitting">
            <span v-if="isSubmitting">{{ author ? 'Updating...' : 'Creating...' }}</span>
            <span v-else>{{ author ? 'Update Author' : 'Create Author' }}</span>
          </button>
        </div>
      </form>
    </div>
...
const submitAuthorForm = async () => {
  ...
  try {
    ...
    let res;
    if (props.author) {
      res = await updateAuthor(props.author.id, authorData);
    } else {
      res = await createAuthor(authorData);
    }
    const updatedAuthor = res.data.author || res.data;
    emit('submit', updatedAuthor);
    Swal.fire({
      toast: true,
      position: 'bottom-end',
      icon: 'success',
